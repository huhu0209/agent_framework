...
1. **Should ARCH-REVIEW.md include findings already documented in CONCERNS.md?**
   - What we know: CONCERNS.md has excellent analysis of all 5 known issues. SECURITY-REVIEW.md existed alongside it.
   - What's unclear: Whether ARCH-REVIEW.md should be self-contained or can reference CONCERNS.md.
   - Recommendation: Per D-01, the 5 known issues are the "骨架" (skeleton). ARCH-REVIEW.md should contain enough context to be readable standalone, but can reference CONCERNS.md for detailed analysis. New findings should be fully documented in ARCH-REVIEW.md.

2. **What is the threshold for recording a finding vs. noting it as acceptable design?**
   - What we know: The codebase is generally well-structured. Some patterns (like `dict[str, Any]` for `extra`) are pragmatic choices, not necessarily bugs.
   - What's unclear: Where to draw the line between "improvement opportunity" and "working as designed".
   - Recommendation: Record anything that would cause a developer to pause and think during maintenance. If it requires a comment to explain "why", it is worth recording. The priority level (HIGH/MEDIUM/LOW) communicates whether action is recommended.
