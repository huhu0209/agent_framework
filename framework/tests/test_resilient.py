...
class TestStreamRetry:
    ...
    @pytest.mark.asyncio
    async def test_stream_empty_first_event_raises_runtime_error(self):
        """H-D2: 首事件即 StopAsyncIteration（空流）应抛 RuntimeError，不当成功、不记 success。"""
        async def empty_stream(config):
            return
            yield  # 使函数成为 async generator，但首 __anext__ 即 StopAsyncIteration
        ...
        with pytest.raises(RuntimeError, match="空流"):
            async for _ in adapter.stream(config):
                pass

    @pytest.mark.asyncio
    async def test_stream_empty_first_event_does_not_record_success(self):
        """H-D2: 空流不污染 breaker success 计数（不当成功）。"""
        async def empty_stream(config):
            return
            yield  # 使函数成为 async generator，但首 __anext__ 即 StopAsyncIteration
        ...
        adapter = ResilientLLMAdapter(provider=mock_provider, breaker=breaker)
        config = CompletionConfig(model="m", messages=[])
        ...
        # 空流是异常，不应被当作成功（不压低 breaker 失败计数）
