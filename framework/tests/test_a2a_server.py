...
class TestCreateTaskRoute:
    ...
    @pytest.mark.asyncio
    async def test_create_task_oversized_body_returns_413(self) -> None:
        """G2: 超过 MAX_BODY 上限的请求返回 413，不耗尽内存。"""
        from agent_framework.a2a.server import _MAX_BODY
        server = make_server()
        send = MockSend()
        big_body = b"x" * (_MAX_BODY + 1)
        await server(mock_scope("POST", "/tasks"), mock_receive(big_body), send)
        status, resp = send.get_response()
        assert status == 413
        assert "过大" in resp["error"] or "too large" in resp["error"].lower()
...
class TestBackgroundExecution:
    ...
    @pytest.mark.asyncio
    async def test_agent_timeout_marks_failed(self, monkeypatch) -> None:
        """H-G2: agent.run 超过 _TASK_TIMEOUT 则 task 标 FAILED。"""
        import agent_framework.a2a.server as server_mod

        class SlowAgent(Agent):
            async def run(self, user_message: str) -> AsyncGenerator[AgentEvent, None]:
                await asyncio.sleep(5)  # 远超测试 timeout
                yield AgentEvent(type="done", step=1, data={"text": "late"})

        monkeypatch.setattr(server_mod, "_TASK_TIMEOUT", 0.1)
        server = make_server(SlowAgent())
        send = MockSend()
        body = json.dumps({"message": "hello"}).encode()
        await server(mock_scope("POST", "/tasks"), mock_receive(body), send)
        _, resp = send.get_response()
        task_id = resp["id"]
        await asyncio.sleep(0.4)  # 等超时 + 状态写入
        task = server._tasks[task_id]
        assert task.status == A2ATaskStatus.FAILED
...
class TestApiKeyAuth:
    ...
    def test_verify_auth_uses_constant_time_compare(self) -> None:
        """G1: _verify_auth 必须用 hmac.compare_digest（恒定时间），防时序攻击。

        恒定时间是非功能安全属性，无法通过行为测试验证，故用源码检查锁定。
        """
        import inspect
        from agent_framework.a2a.server import A2AServer
        source = inspect.getsource(A2AServer._verify_auth)
        assert "hmac.compare_digest" in source


def test_server_init_creates_concurrency_semaphore() -> None:
    """H-G2: A2AServer 实例持有 Semaphore 限并发（源码检查锁定）。"""
    import inspect
    source = inspect.getsource(A2AServer.__init__)
...
class TestTaskCleanup:
    @pytest.mark.asyncio
    async def test_terminal_task_cleaned_after_retention(self, monkeypatch) -> None:
        """H-G1: 终态 task 在 _TASK_RETENTION_SECONDS 后从 _tasks 删除。"""
        import agent_framework.a2a.server as server_mod
        monkeypatch.setattr(server_mod, "_TASK_RETENTION_SECONDS", 0.1)
        ...
        await asyncio.sleep(0.1)  # 等 agent 完成（COMPLETED）
        assert task_id in server._tasks  # retention 窗口内仍在

        await asyncio.sleep(0.2)  # 等 retention 过期 + cleanup 执行
        assert task_id not in server._tasks  # 已清理

    @pytest.mark.asyncio
    async def test_create_task_over_max_returns_503(self, monkeypatch) -> None:
        """H-G1: 超 _MAX_TASKS 上限时返回 503 拒绝（fail-fast）。"""
        import agent_framework.a2a.server as server_mod
        monkeypatch.setattr(server_mod, "_MAX_TASKS", 1)

        server = make_server()
        send1 = MockSend()
        await server(
            mock_scope("POST", "/tasks"),
            mock_receive(json.dumps({"message": "a"}).encode()),
            send1,
        )
        assert send1.get_response()[0] == 201

        send2 = MockSend()
        await server(
            mock_scope("POST", "/tasks"),
            mock_receive(json.dumps({"message": "b"}).encode()),
            send2,
        )
        status, resp = send2.get_response()
        assert status == 503
