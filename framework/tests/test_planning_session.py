...
class TestCreateFromItems:
    def test_has_plan_after_create(self) -> None:
        ...
        session.create_from_items(_items("step A", "step B"), source="caller_injected")
...
class TestReplan:
    def test_allow_replan_overwrites(self) -> None:
        ...
        session.create_from_items(_items("old"), source="caller_injected")
        text = "<plan>\n1. new step\n</plan>"
        assert session.try_parse_from_response(text) is True
        snap = session.snapshot()
        assert snap is not None
...
class TestUpdateStatus:
    def test_pending_to_in_progress(self) -> None:
        session = PlanningSession()
        session.create_from_items(_items("step A"), source="caller_injected")
...
# ---------------------------------------------------------------------------
# 7b. advance — reset_drift 兜底（H-E2）
# ---------------------------------------------------------------------------

class TestAdvanceDrift:
    """H-E2: advance() 接入 reset_drift 兜底。"""

    def test_advance_resets_drift_when_active_item(self) -> None:
        """有 in_progress 项时 advance 归零 drift（兜底，不依赖 LLM 调 update_status）。"""
        session = PlanningSession(drift_warn=3, drift_abort=8)
        session.create_from_items(_items("step1"), source="caller_injected")
        session.update_status("1", "in_progress")  # 制造活跃项
        # 累加 drift（模拟 LLM 没调 update_status 完成它）
        session.increment_drift()
        session.increment_drift()
        assert session.drift_count == 2

        # advance 检测到有活跃项 → reset_drift 归零
        level = session.advance()
        assert level == DriftLevel.NONE
        assert session.drift_count == 0

    def test_advance_accumulates_drift_when_all_pending(self) -> None:
        """全 pending 不推进时 advance 累加 drift 到 ABORT（误中止场景仍正确）。"""
        session = PlanningSession(drift_warn=3, drift_abort=8)
        session.create_from_items(_items("step1"), source="caller_injected")
        ...
        for _ in range(8):
            level = session.advance()
        assert level == DriftLevel.ABORT
        assert session.drift_count == 8
