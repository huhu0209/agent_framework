...
# ---------------------------------------------------------------------------
# AgentConfig.from_loader
# ---------------------------------------------------------------------------

class TestAgentConfigFromLoader:
    """AgentConfig.from_loader(loader) factory method tests."""

    def _write_agent_md(self, directory: Path, name: str, prompt: str,
                        model: str = "claude-sonnet-4-6-20250514",
                        max_steps: int = 10) -> Path:
        """Helper: write an agent .md file and return its path."""
        content = (
            f"---\nname: {name}\nmodel: {model}\n"
    ...
    def test_from_loader_no_paths_returns_empty(self):
        """discover() returns [] => from_loader returns {}."""
        loader = ConfigLoader(global_dir=Path("/nonexistent"), project_dir=Path("/nonexistent"))
        result = AgentConfig.from_loader(loader)
        assert result == {}

    def test_from_loader_global_only(self, tmp_path: Path):
        """Only global directory exists => all agents from that dir returned."""
        global_dir = tmp_path / "global" / ".agent-framework" / "agents"
        global_dir.mkdir(parents=True)
        self._write_agent_md(global_dir, "agent-a", "Prompt A")
        self._write_agent_md(global_dir, "agent-b", "Prompt B")
        ...
        assert result["agent-a"].system_prompt == "Prompt A"
        assert result["agent-b"].system_prompt == "Prompt B"

    def test_from_loader_global_and_project_disjoint(self, tmp_path: Path):
        """Global and project have disjoint agent names => union returned."""
        global_dir = tmp_path / "global" / ".agent-framework" / "agents"
        global_dir.mkdir(parents=True)
        project_dir = tmp_path / "project" / ".agent-framework" / "agents"
        project_dir.mkdir(parents=True)

        self._write_agent_md(global_dir, "global-agent", "Global prompt")
        self._write_agent_md(project_dir, "project-agent", "Project prompt")
        ...
        result = AgentConfig.from_loader(loader)
        assert set(result.keys()) == {"global-agent", "project-agent"}

    def test_from_loader_project_overrides_global_on_collision(self, tmp_path: Path):
        """Same agent name in global and project => project wins."""
        global_dir = tmp_path / "global" / ".agent-framework" / "agents"
        global_dir.mkdir(parents=True)
        project_dir = tmp_path / "project" / ".agent-framework" / "agents"
        project_dir.mkdir(parents=True)

        self._write_agent_md(global_dir, "shared", "Global version")
        self._write_agent_md(project_dir, "shared", "Project version")
        ...
        assert result["shared"].system_prompt == "Project version"
        assert len(result) == 1

    def test_from_loader_returns_dict_of_agent_config(self, tmp_path: Path):
        """Return type is dict[str, AgentConfig]."""
        global_dir = tmp_path / "global" / ".agent-framework" / "agents"
        global_dir.mkdir(parents=True)
        self._write_agent_md(global_dir, "typed", "Typed prompt")
        ...
        assert all(isinstance(v, AgentConfig) for v in result.values())

    def test_from_loader_collision_emits_warning(self, tmp_path: Path, caplog):
        """Name collision emits a logger.warning."""
        global_dir = tmp_path / "global" / ".agent-framework" / "agents"
        global_dir.mkdir(parents=True)
        project_dir = tmp_path / "project" / ".agent-framework" / "agents"
        project_dir.mkdir(parents=True)

        self._write_agent_md(global_dir, "dup", "Global")
        self._write_agent_md(project_dir, "dup", "Project")
        ...
        assert result["dup"].system_prompt == "Project"
        # Check a warning was emitted about the override
        assert any("overrides" in record.message.lower() or "dup" in record.message for record in caplog.records)
