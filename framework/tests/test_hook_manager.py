...
# --- from_loader factory method tests ---


def _write_hooks_json(hooks_dir: Path, hooks_data: str) -> Path:
    """Write hooks.json into a hooks directory and return its path."""
    hooks_dir.mkdir(parents=True, exist_ok=True)
    hooks_json = hooks_dir / "hooks.json"
    hooks_json.write_text(hooks_data, encoding="utf-8")
    return hooks_json


def test_from_loader_no_dirs(tmp_path):
    """No dirs — returns empty manager, fire returns empty list."""
    loader = ConfigLoader(
    ...
    assert manager.trusted is False


def test_from_loader_global_only(tmp_path):
    """Global hooks.json loaded."""
    global_hooks = tmp_path / "home" / ".agent-framework" / "hooks"
    _write_hooks_json(global_hooks, """{
        "hooks": {
            "PreToolUse": [
                {"matcher": "Write", "hooks": [{"type": "command", "command": "echo check"}]}
            ]
        }
    }""")
    ...
    assert manager._hooks[HookEvent.PRE_TOOL_USE][0].command == "echo check"


def test_from_loader_both_dirs(tmp_path):
    """Global PreToolUse + project PostToolUse — both registered."""
    global_hooks = tmp_path / "home" / ".agent-framework" / "hooks"
    project_hooks = tmp_path / "project" / ".agent-framework" / "hooks"
    _write_hooks_json(global_hooks, """{
        "hooks": {
            "PreToolUse": [
                {"matcher": "*", "hooks": [{"type": "command", "command": "echo global-pre"}]}
            ]
        }
    }""")
    _write_hooks_json(project_hooks, """{
        "hooks": {
            "PostToolUse": [
                {"matcher": "*", "hooks": [{"type": "command", "command": "echo project-post"}]}
            ]
        }
    }""")
...
def test_from_loader_same_event_both_dirs(tmp_path):
    """Both dirs have PreToolUse — both registered, global first."""
    global_hooks = tmp_path / "home" / ".agent-framework" / "hooks"
    project_hooks = tmp_path / "project" / ".agent-framework" / "hooks"
    _write_hooks_json(global_hooks, """{
        "hooks": {
    ...
    manager = HookManager.from_loader(loader)
    pre_hooks = manager._hooks[HookEvent.PRE_TOOL_USE]
    assert len(pre_hooks) == 2
    assert pre_hooks[0].command == "echo global"
    assert pre_hooks[1].command == "echo project"


def test_from_loader_dir_exists_no_json(tmp_path):
    """Hooks dir exists but no hooks.json — manager is empty."""
    global_hooks = tmp_path / "home" / ".agent-framework" / "hooks"
...
def test_from_loader_trusted_default_false(tmp_path):
...
def test_from_loader_trusted_true(tmp_path):
    """trusted=True passes through to constructor."""
    loader = ConfigLoader(
        global_dir=tmp_path / "home",
        project_dir=tmp_path / "project",
    )
    manager = HookManager.from_loader(loader, trusted=True)
    assert manager.trusted is True
...
@pytest.mark.asyncio
async def test_fire_concurrent_register_no_runtime_error(monkeypatch):
    """H-S4: fire() 在 await 期间并发 register 不抛 RuntimeError（迭代列表快照）。"""
    mgr = HookManager(trusted=True)
    mgr.register(HookConfig(
        event=HookEvent.PRE_TOOL_USE,
        matcher="*",
    ...
    original_exec = mgr._execute_command
    registered_during_await = []

    async def _slow_execute(config, context):
        # 第一次调用时，await 让出控制权期间触发 register 改变 _hooks[event]
        if not registered_during_await:
            registered_during_await.append(True)
            mgr.register(HookConfig(
                event=HookEvent.PRE_TOOL_USE,
                matcher="*",
                hook_type=HookType.COMMAND,
        ...
        return await original_exec(config, context)

    monkeypatch.setattr(mgr, "_execute_command", _slow_execute)

    # 修复前：迭代 self._hooks[event] 期间列表被 append → RuntimeError
    # 修复后：迭代快照 list(...)，不抛
    results = await mgr.fire(
        HookEvent.PRE_TOOL_USE,
        HookContext(hook_event_name="PreToolUse", tool_name="echo"),
    )
    # 快照时只有 1 个 hook，故只执行 1 次（新 register 的不影响本次迭代）
    assert len(results) == 1
