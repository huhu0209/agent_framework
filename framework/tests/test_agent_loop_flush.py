...
@pytest.mark.asyncio
async def test_compact_cascade_writes_semantic_memory(memory_dir: Path):
    """compact 触发后，cascade 提取的语义记忆应真正写入文件。

    回归：agent_loop 中 SemanticWriter(...).write_batch(drafts) 曾漏 await，
    导致 coroutine 从不执行、文件不落盘。
    """
    from agent_framework.memory.types import MemoryType, SemanticMemoryDraft

    draft = SemanticMemoryDraft(
        name="cascade bug regression",
        description="回归测试",
        type=MemoryType.USER,
        body="验证 compact cascade 是否真正落盘。",
    )
    extractor = AsyncMock()
    extractor.extract_from_messages.return_value = [draft]
    ...
    with (
        ...
        await loop._maybe_compact(messages, step=1)

    semantic_files = list(memory_dir.glob("user_*.md"))
    assert semantic_files, (
        f"语义记忆文件未写入 — write_batch 可能未被 await。memory_dir 内容: "
        f"{list(memory_dir.iterdir())}"
    )
