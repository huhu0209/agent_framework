...
@pytest.mark.asyncio
async def test_run_subagent_isolates_ctx_from_parent(monkeypatch) -> None:
    """E1: run_subagent 给子 AgentLoop 隔离的 ctx（独立 extra dict），防覆盖父级 planning_session。"""
    captured: list = []
    ...
    adapter = FakeAdapter(text="子代理完成")
    router = _make_router("safe_tool")
    parent_ctx = ToolUseContext(extra={"planning_session": "PARENT_SESSION"})
    ...
    assert child_ctx is not parent_ctx  # 隔离对象，非同一引用
    assert child_ctx.extra is not parent_ctx.extra  # 独立 extra dict
    assert child_ctx.extra.get("planning_session") == "PARENT_SESSION"  # 内容继承
    assert parent_ctx.extra["planning_session"] == "PARENT_SESSION"
