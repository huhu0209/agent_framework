...
class TestFromLoader:
    """CommandDispatcher.from_loader() factory method tests."""

    def test_from_loader_no_skill_dirs(self, tmp_path):
        """No skill dirs — dispatcher resolves unknown to NONE."""
        loader = ConfigLoader(
            global_dir=tmp_path / "home",
            project_dir=tmp_path / "project",
        )
        dispatcher = CommandDispatcher.from_loader(loader)
        result = dispatcher.resolve("/unknown")
        assert result.action == CommandAction.NONE

    def test_from_loader_with_skills(self, tmp_path):
        """Global dir has a user-invocable skill — dispatcher resolves it."""
        global_skills = tmp_path / "home" / ".agent-framework" / "skills"
        global_skills.mkdir(parents=True)
        create_skill(global_skills, "test-skill", "测试 skill", "body")
        ...
        dispatcher = CommandDispatcher.from_loader(loader)
        result = dispatcher.resolve("/test-skill")
        assert result.action == CommandAction.LOAD_SKILL

    def test_from_loader_returns_dispatcher(self, tmp_path):
        """Return type is CommandDispatcher."""
        loader = ConfigLoader(
            global_dir=tmp_path / "home",
            project_dir=tmp_path / "project",
        )
    ...
    def test_from_loader_skill_registry_populated(self, tmp_path):
        """Dispatcher created via from_loader has skill registry with expected names."""
        global_skills = tmp_path / "home" / ".agent-framework" / "skills"
        global_skills.mkdir(parents=True)
        create_skill(global_skills, "deploy", "部署", "部署: $ARGUMENTS")
        ...
        dispatcher = CommandDispatcher.from_loader(loader)
        result = dispatcher.resolve("/deploy staging")
        assert result.action == CommandAction.LOAD_SKILL
        assert "staging" in result.skill_content

    def test_existing_constructor_still_works(self, tmp_path):
        """Existing constructor signatures unchanged (ADP-09)."""
        # None registry
        d1 = CommandDispatcher(skill_registry=None)
        assert d1.resolve("/unknown").action == CommandAction.NONE

        # Explicit registry
        skills_dir = tmp_path / "skills"
        skills_dir.mkdir()
        create_skill(skills_dir, "hello", "Hello skill", "body")
        registry = SkillRegistry([skills_dir])
        d2 = CommandDispatcher(skill_registry=registry)
        assert d2.resolve("/hello").action == CommandAction.LOAD_SKILL

        # Default (no args)
        d3 = CommandDispatcher()
        assert d3.resolve("/help").action == CommandAction.SHOW_HELP
