...
class TestParseSseLines:
    ...
    @pytest.mark.asyncio
    async def test_invalid_json_logs_debug(self, caplog) -> None:
        """H-D1: 坏 JSON chunk 触发 logger.debug，不再静默丢弃。"""
        import logging

        lines = ['data: not json at all', 'data: {"valid": true}']
        with caplog.at_level(logging.DEBUG, logger="agent_framework.llm.streaming"):
            result = await self._collect(lines)
